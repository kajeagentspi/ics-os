...
void getstring(char *buf, DEX32_DDL_INFO *dev)
{
      ...
      do
      {
            ...
            {
                  if (i < 256)
                  { //maximum command line is only 255 characters
...
int user_fork()
{
      ...
      {                   //wait for the process to be dispatched before returning
            taskswitch(); //try to wakeup process_dispatcher()
            ;
      }

      if (curval != current_process->processid)
      { //this is the child
...
int user_execp(char *fname, DWORD mode, char *params)
{
      ...
      if (f != 0)
      {                                                   //The executable was successfully opened
            ...
            { //A successful read of the executable
                  char temp[255];
...
void console_ls(int style, int sortmethod)
{
      ...
      for (i = 0; i < totalfiles; i++)
      {
            ...
            { //list view style
                  ...
                  textcolor(WHITE);
                  printf("%10d %14s %14s\n", buffer[i].size,
                         vfs_attbstr(&buffer[i], temp), datetostr(&buffer[i].date_modified, mdatestr));
...
int console_execute(const char *str)
{
      ...
      { //-- Show process information. Args: <pid>
            int pid;
            u = strtok(0, " ");
            if (u != 0)
            {
      ...
      { //-- Shows a list of path aliases.
            vfs_showpathcuts();
      }
      else if (strcmp(u, "pcut") == 0)
      { //-- Creates a path alias. Args: <alias:> [path]
            char *u2, *u3;
            u2 = strtok(0, " ");
            u3 = strtok(0, " ");
            if (u2 != 0)
      ...
      { //-- Removes a directory and all its subdirectories. Args: <dirname>
            char *u2 = strtok(0, " ");
            if (u2 != 0)
            {
                  char c;
      ...
      { //-- Removes a path alias. Args: <alias:>
            char *u2;
            u2 = strtok(0, " ");
            if (u2 != 0)
            {
      ...
      { //-- Exits a console session.
            fg_exit();
            exit(0);
      }
      else if (strcmp(u, "echo") == 0)
      { //-- Displays a string. Args: <string>
            u = strtok(0, "\n");
            if (u != 0)
                  printf("%s\n", u);
      }
      else if (strcmp(u, "use") == 0)
      { //-- Tells the extension manager to use the extension: Args: <extension>
            u = strtok(0, " ");
      ...
      { //-- Shows list of currently open files.
            file_showopenfiles();
      }
      ...
      else if (strcmp(u, "dkill") == 0)
      { //-- Dirty kill a user process/thread. No cleanup is done. Args: <pid>
            u = strtok(0, " ");
            if (u != 0)
            {
                  ps_user_kill(atoi(u));
      ...
      { //-- Kills a thread/process. Performs cleanup.  Args: <thread name>
            u = strtok(0, " ");
            if (u != 0)
            {
      ...
      { //-- List the running processes. "ps" can also be used.
            show_process();
      }
      else if (strcmp(u, "cls") == 0 || strcmp(u, "clear") == 0)
      { //-- Clears the screen.
            clrscr();
      ...
      { //-- Unmounts a mounted device. Args: <mount point>
            char *u = strtok(0, " ");
            if (u != 0)
            {
      ...
      { //-- Mounts a device. Args: fat/cdfs <partition/device> <mount point>
      ...
      else if (strcmp(u, "pwd") == 0)
      { //-- Shows the current working directory.
            char temp[255];
            printf("%s\n", showpath(temp));
      }
      else if (strcmp(u, "lsmod") == 0)
      { //-- Shows the list of loaded libraries and modules.
            showlibinfo();
      }
      ...
      { //-- Creates a directory. Args: <directory name>
            u = strtok(0, " ");
            if (u != 0)
            {
                  if (mkdir(u) == -1)
      ...
      { //-- Executes a batch file or script. Args: <script>
            u = strtok(0, " ");
            if (u != 0)
            {
                  if (script_load(u) == -1)
      ...
      { //-- Shows directory listing. Args: [-l | -osize | -oname]
      ...
      { //-- Deletes a files or directory. Args: <filename/dirname>
            int res;
            u = strtok(0, " ");
            if (u != 0)
            {
      ...
      { //-- Renames a file. Args: <oldname> <newname>
            char *u2, *u3;
            u2 = strtok(0, " ");
            u3 = strtok(0, " ");
            if (u2 != 0 && u3 != 0)
      ...
      { //-- Displays the contents of a file. Args: <filename> [-p]
            u = strtok(0, " ");
            if (u != 0)
            {
      ...
      else if (strcmp(u, "copy") == 0 || strcmp(u, "cp") == 0)
      { //-- Copy source to destination: Args: <source> <destination>
            u = strtok(0, " ");
            if (u != 0)
            {
                  char *u2 = strtok(0, " ");
      ...
      { //-- Changes working directory. Args: <directory>
            u = strtok(0, " ");
            if (u != 0)
            {
      ...
      { //-- Loads a shared library (.dll or .so). Args: <module filename>
            u = strtok(0, " ");
            if (u != 0)
            {
                  if (loadDLL(u, str) == -1)
      ...
      { //-- Lists all modules currently installed and available.
            devmgr_showdevices();
      }
      ...
      { //-- Sets an environment variable. Args: <key>=<value>
            u = strtok(0, " ");
            if (u == 0)
            {
                  env_showenv();
      ...
      { //-- Unloads a library. Args: <library name>
            u = strtok(0, " ");
            if (u != 0)
            {
      ...
      { //-- Runs the graphics demonstration.
            demo_graphics();
      }
      else if (strcmp(u, "cc") == 0)
      { //-- Builds a C program (invokes tcc.exe). Args: <name.exe> <name.c>
      ...
      else if (strcmp(u, "cal") == 0)
      {
            cal(args);
      }
      else if (strcmp(u, "cat") == 0)
      {
            cat(args);
      }
      ...
      { //ok it is not a command, maybe it's an executable?
            if (u != 0)
            {
